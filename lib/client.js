...
Voog.prototype._multipart_post = function(path, parameters, callback) {
  var parsedUrl = url.parse(path),
      postUrl;
  if (parsedUrl.host) {
      postUrl = parsedUrl;
  ...
  form.append('filename', parameters.filename);
};
...
Voog.prototype._s3upload = function(path, file, callback) {
    var uploadUrl = (path.indexOf('://') > -1 ? path : this.protocol + this.host + API_URL_ENDPOINT + path),
